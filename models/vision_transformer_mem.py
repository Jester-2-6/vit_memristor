    ...
    def __init__(
        ...
        if conv_stem_configs is not None:
            ...
            seq_proj.add_module(
                "conv_last",
                memConv2d(
                    in_channels=prev_channels, out_channels=hidden_dim, kernel_size=1, lookup_table=lut
                ),
            )
            self.conv_proj: nn.Module = seq_proj
        else:
            self.conv_proj = memConv2d(
                in_channels=3,
        ...
        if representation_size is None:
            heads_layers["head"] = memLinear(hidden_dim, num_classes, lookup_table=lut)
        else:
